...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 md:p-12 lg:p-24 selection:bg-brand selection:text-white">
      ...
      <main className="relative max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-16 items-start">
        <div className="lg:col-span-1 lg:sticky lg:top-12 lg:h-[calc(100vh-6rem)] flex flex-col space-y-8">
          {!process.env.GEMINI_API_KEY && (
            <motion.div 
          ...
          <header className="space-y-6">
            <div className="space-y-4">
              ...
              >
                ...
                <h1 className="text-3xl font-extrabold tracking-tighter text-white mb-0 leading-none not-italic">BearTag</h1>
              </motion.div>
              <motion.p 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
          ...
          {/* Upload Section */}
          <section className="flex-1 flex flex-col space-y-6 min-h-0">
            <motion.div
              initial={{ opacity: 0, scale: 0.98 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 0.2 }}
              className={cn(
                "glass-card flex-1 p-8 flex flex-col items-center justify-center border-2 border-dashed transition-all duration-300 relative min-h-[300px]",
            ...
            >
              ...
              <div className="text-center space-y-2">
                <p className="text-lg font-medium">Drag & drop images</p>
              ...
              />
              <div className="mt-6 pt-6 border-t border-gray-800 w-full flex flex-col items-center justify-center gap-2">
            ...
            >
              <div className="relative group">
                <LinkIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 group-focus-within:text-brand transition-colors" />
                <input 
                  type="text" 
                  ...
                  className="w-full bg-gray-900/50 border border-gray-800 rounded-xl py-3 pl-10 pr-10 text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand transition-all"
                />
                {urlInput && (
                  <button 
                    onClick={() => setUrlInput("")}
              ...
              <button 
                ...
                className="w-full bg-brand text-black py-3 rounded-xl font-bold hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(0,169,206,0.2)]"
              >
                Add URL
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {items.length > 0 && (
              <motion.button
                ...
                className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-gray-400 hover:text-red-400 transition-colors px-3 py-1.5 bg-gray-900 rounded-lg border border-gray-800"
              >
                <X className="w-3 h-3" />
                Clear All
              </motion.button>
            ...
            <AnimatePresence mode="popLayout">
              {items.length === 0 ? (
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="h-[400px] flex flex-col items-center justify-center text-gray-600 border border-dashed border-gray-800 rounded-3xl"
                >
                ...
                items.map((item, index) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row">
                      {/* Thumbnail */}
                      <div className="w-full md:w-56 h-40 md:h-auto bg-gray-950 flex-shrink-0 relative border-r border-gray-800">
                        <img 
                          src={item.source} 
                          alt="Thumbnail" 
                      ...
                      <div className="flex-1 p-5 flex flex-col justify-between space-y-3">
                        <div className="space-y-3">
                          ...
                          <div className={cn(
                            "p-3.5 rounded-xl font-mono text-xs md:text-sm leading-relaxed min-h-[60px] flex items-center transition-colors",
                        ...
                        {item.status === 'done' && (
                          <div className="flex justify-end">
                            <button 
                              ...
                              className={cn(
                                "flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-all",
                                copiedId === item.id 
